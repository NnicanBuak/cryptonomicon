    ...
    <div class="container">
      <section>
        ...
        <button
          ...
          type="button"
          class="m-4 inline-flex items-center py-2 px-4 border border-transparent shadow-sm text-sm leading-4 font-medium rounded-full text-white bg-green-400 hover:bg-green-500 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500"
        >
      ...
      <template v-if="tickers.length > 0">
        <section>
          ...
          <div
            :class="{ 'pb-8': this.tickers.length <= this.tickersMax }"
          ...
          >
            ...
            <div
              v-if="this.tickers.length > this.tickersMax"
              class="pagination flex p-2 gap-2"
            >
              <div>
                <button
			...
			C240.57,112.315,240.57,104.628,235.878,99.876z"
                ...
                <span class="text-xl"
                  >{{ this.tickersPage }}/{{
                    Math.round(this.tickers.length / this.tickersMax)
                  }}</span
                >
                <button
		...
		S57.087,46.51,51.797,51.798z"
              ...
              <div><select ref="tickersMax" name="pagination"></select></div>
            </div>
          </div>
          <dl
            class="pb-8 grid grid-cols-1 gap-5 bg-gray-100 sm:grid-cols-3 px-4"
          >
            <div
      ...
      <section v-show="selectedTickerName !== null" class="relative mx-4">
        ...
        <div
          ...
          :alt="this.graphScale"
          class="overflow-hidden flex flex-row-reverse border-gray-600 border-b border-r h-64 cursor-auto"
        >
          <div
...
<script>
// Проблемы и их критичность:
// 1. 🦆🦆🦆🦆🦆🦆 | Наличие в состоянии ЗАВИСИМОСТЕЙ |[👍]

// 2. 🦆🦆🦆🦆🦆 | При удалении остаётся подписка на обновление тикера |[]

// 3. 🦆🦆🦆🦆🦆 | Обработка ошибок API |[]

// 4. 🦆🦆🦆🦆 | Количество запросов |[]

// 5. 🦆🦆🦆🦆 | Запросы внутри компонента |[]

// 6. 🦆🦆🦆🦆 | При удалении тикера не удаляется loackStorage |[👍]

// 7. 🦆🦆🦆 | Повторения в watch |[👍]

// 8. 🦆🦆🦆 | localStorage и анонимные вкладки |[]

// 9. 🦆🦆 | Неудобный вывод графика |[]
